import { AlertCircle, FolderOpen, Loader2, LogIn, Upload } from "lucide-react";
...
export default function CertificatesFolder() {
  ...
  const { data: uploadedCertificates = [], isLoading: certificatesLoading } =
    useGetAllCertificates();
  const uploadMutation = useUploadCertificate();

  const allCertificates: Certificate[] = uploadedCertificates.map((cert) => ({
  ...
  const handleNavigate = (direction: "prev" | "next") => {
    ...
    if (direction === "prev" && selectedIndex > 0) {
      ...
      direction === "next" &&
      selectedIndex < allCertificates.length - 1
    ) {
      setSelectedIndex(selectedIndex + 1);
    }
  };
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
            {allCertificates.map((cert, index) => (
              <button
                key={cert.path || index}
                type="button"
                className="group relative bg-card rounded-xl overflow-hidden shadow-md border border-border hover:shadow-xl hover:border-teal-500/50 transition-all duration-300 ease-in-out cursor-pointer text-left w-full"
